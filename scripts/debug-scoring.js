...
import { normalizeTitle, normalizeAuthor } from '../src/matching/utils/normalization.js';
...
const titleSimilarity = calculateTextSimilarity(normalizedTargetTitle, normalizedResultTitle);
const authorSimilarity = calculateTextSimilarity(normalizedTargetAuthor, normalizedResultAuthor);

console.log('📊 Similarity Scores:');
console.log('  Title Similarity:', titleSimilarity.toFixed(4), `(${(titleSimilarity * 100).toFixed(1)}%)`);
console.log('  Author Similarity:', authorSimilarity.toFixed(4), `(${(authorSimilarity * 100).toFixed(1)}%)`);
console.log('');
...
Object.entries(result.breakdown).forEach(([key, value]) => {
  console.log(`  ${key}:`, value.score?.toFixed(2) || 'N/A', 
...
console.log('  "test" vs "test":', simpleResult, `(${(simpleResult * 100).toFixed(1)}%)`);

const simpleResult2 = calculateTextSimilarity('The Laws of the Skies', 'The Laws of the Skies');
console.log('  Exact title match:', simpleResult2, `(${(simpleResult2 * 100).toFixed(1)}%)`);

const simpleResult3 = calculateTextSimilarity('Gregoire Courtois', 'Gregoire Courtois');
console.log('  Exact author match:', simpleResult3, `(${(simpleResult3 * 100).toFixed(1)}%)`);
