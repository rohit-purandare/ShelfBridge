...
export class Config {
  ...
  _loadConfig() {
    if (!fs.existsSync(this.configPath)) {
      ...
      logger.warn(
        `Config file not found: ${this.configPath}, will use environment variables and defaults`,
      );
      this.globalConfig = {};
      this.users = [];
      return;
  ...
  _loadUsersFromEnvironment() {
    ...
    const maxUserIndex = Math.max(
      -1,
      ...Object.keys(userEnvVars).map(i => parseInt(i)),
    );
      ...
      if (this.users[i]) {
        ...
        for (const [envKey, envValue] of Object.entries(envUser)) {
          ...
          if (configKey && this.users[i][configKey] === undefined) {
            const parsedValue = this._parseEnvironmentValue(
              envValue,
              configKey,
            );
            if (parsedValue !== null) {
              ...
              logger.debug(
                `Set user ${i} ${configKey} from environment: ${configKey === 'abs_token' || configKey === 'hardcover_token' ? '[REDACTED]' : parsedValue}`,
              );
            }
          }
        }
        ...
        if (
          ...
          logger.debug(
            `Created user ${i} from environment variables: ${newUser.id}`,
          );
        }
      }
    }
  ...
  _applyDefaults() {
    ...
    for (const [key, defaultValue] of Object.entries(defaults)) {
      if (this.globalConfig[key] === undefined) {
        ...
        logger.debug(
          `Using explicit value for ${key}: ${this.globalConfig[key]}`,
        );
      }
    }
    ...
    if (this.globalConfig.max_books_to_process !== undefined) {
      ...
      logger.debug(
        `Using explicit value for max_books_to_process: ${this.globalConfig.max_books_to_process}`,
      );
    }
    if (this.globalConfig.max_books_to_fetch !== undefined) {
      this.explicitlySet.add('max_books_to_fetch');
      logger.debug(
        `Using explicit value for max_books_to_fetch: ${this.globalConfig.max_books_to_fetch}`,
      );
    }
    if (this.globalConfig.page_size !== undefined) {
      this.explicitlySet.add('page_size');
      logger.debug(
        `Using explicit value for page_size: ${this.globalConfig.page_size}`,
      );
    }
  }
