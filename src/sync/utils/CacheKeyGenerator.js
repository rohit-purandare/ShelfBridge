...
export class CacheKeyGenerator {
  ...
  static generatePossibleKeys(identifiers, hardcoverMatch) {
    ...
    try {
      ...
      logger.debug('Generated cache keys', {
        ...
        keys: possibleCacheKeys.map(k => `${k.type}:${k.key.substring(0, 20)}...`)
      });
  ...
  static generateFallbackKey(title, author) {
    try {
      ...
      const safeTitle = (title === null || title === undefined) ? 'unknown' : title;
      const safeAuthor = (author === null || author === undefined) ? 'unknown' : author;
  ...
  static generateSyntheticKey(userBookId, editionId) {
    try {
      // Convert null/undefined to 'unknown' upfront
      const safeUserBookId = (userBookId === null || userBookId === undefined) ? 'unknown' : userBookId;
      const safeEditionId = (editionId === null || editionId === undefined) ? 'unknown' : editionId;
