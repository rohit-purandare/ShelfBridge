...
describe('TimestampFormatter', () => {
  ...
  describe('formatForDisplay()', () => {
    it('should format ISO string timestamps correctly', () => {
      ...
      assert(
        result.includes('-05') ||
          result.includes('-04') ||
          result.includes('EST'),
        'Should include EST timezone info',
      );
    ...
    it('should return Invalid timestamp for invalid inputs', () => {
      assert.strictEqual(
        formatter.formatForDisplay('invalid-date'),
        'Invalid timestamp',
      );
      assert.strictEqual(
        formatter.formatForDisplay('not-a-number'),
        'Invalid timestamp',
      );
    ...
    it('should respect timezone configuration', () => {
      ...
      assert(
        utcResult.includes('UTC') ||
          utcResult.includes('+00') ||
          utcResult.includes('Z'),
        'UTC result should show UTC timezone',
      );
      assert(
        nyResult.includes('-05') ||
          nyResult.includes('-04') ||
          nyResult.includes('EST'),
        'NY result should show EST timezone',
      );
    });
  });
  ...
  describe('formatForHardcover()', () => {
    ...
    it('should respect timezone for date boundaries', () => {
      ...
      assert.strictEqual(
        nyDate,
        '2024-01-15',
        'NY should be Jan 15 (still same day)',
      );
      assert.strictEqual(
        tokyoDate,
        '2024-01-16',
        'Tokyo should be Jan 16 (next day)',
      );
    });
  });
  ...
  describe('isValidTimestamp()', () => {
    it('should validate ISO string timestamps', () => {
      ...
      assert.strictEqual(
        formatter.isValidTimestamp('2024-01-15T14:30:00Z'),
        true,
      );
      assert.strictEqual(
        formatter.isValidTimestamp('2024-01-15 14:30:00'),
        true,
      );
    });
    ...
    it('should validate string representations of milliseconds', () => {
      assert.strictEqual(
        formatter.isValidTimestamp(testMillis.toString()),
        true,
      );
    });
  ...
  describe('timezone management', () => {
    ...
    it('should apply new timezone to formatting', () => {
      ...
      assert.notStrictEqual(
        originalResult,
        newResult,
        'Changing timezone should affect output',
      );
    });
  });
  ...
  describe('error handling', () => {
    it('should handle malformed ISO strings gracefully', () => {
      ...
      assert.strictEqual(
        formatter.formatForDisplay(malformed),
        'Invalid timestamp',
      );
    ...
    it('should handle timezone changes mid-operation', () => {
      ...
      assert.notStrictEqual(
        result1,
        result2,
        'Timezone changes should be reflected immediately',
      );
    });
  });
});
