...
describe('Title/Author Identifier Consistency', () => {
  it('should generate consistent identifiers for title/author matching', async () => {
    ...
    try {
      ...
      const cacheIdentifier = bookCache.generateTitleAuthorIdentifier(
        title,
        author,
      );
      ...
      assert.match(
        cacheIdentifier,
        expectedPattern,
        'Should follow consistent identifier pattern',
      );
      ...
      const testCases = [
        {
          title: 'Book Title',
          author: 'Author Name',
          expected: 'title_author:book_title|author_name',
        },
        {
          title: 'Book: With Punctuation!',
          author: 'Author-Name',
          expected: 'title_author:book:_with_punctuation!|author-name',
        },
        {
          title: 'Book   With  Spaces',
          author: 'Author  With  Spaces',
          expected: 'title_author:book_with_spaces|author_with_spaces',
        },
      ];

      for (const testCase of testCases) {
        const generated = bookCache.generateTitleAuthorIdentifier(
          testCase.title,
          testCase.author,
        );
        assert.strictEqual(
          generated,
          testCase.expected,
          `Should normalize "${testCase.title}" by "${testCase.author}" correctly`,
        );
  ...
  it('should demonstrate cache lookup consistency', async () => {
    ...
    try {
      ...
      assert.strictEqual(
        cachedInfo.exists,
        true,
        'Should find cached book with consistent identifier',
      );
      ...
      assert.strictEqual(
        progressUnchanged,
        false,
        'Should detect unchanged progress correctly',
      );
