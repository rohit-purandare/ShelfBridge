...
test('Double-Processing Prevention', async t => {
  ...
  await t.test('Race Condition Prevention', async t => {
    ...
    await t.test(
      'should prevent concurrent processing of same book',
      async () => {
    ...
    await t.test(
      'should allow processing different books concurrently',
      async () => {
        ...
        assert.notStrictEqual(
          result1.status,
          'skipped',
          'Book 1 should not be skipped',
        );
        assert.notStrictEqual(
          result2.status,
          'skipped',
          'Book 2 should not be skipped',
        );
      },
    );
    ...
    await t.test('should clean up tracking after processing', async () => {
      ...
      assert.ok(
        !syncManager.booksBeingProcessed.has(bookKey),
        'Book should be removed from tracking',
      );
    });
  });
  ...
  await t.test('Integration with Sync Process', async t => {
    ...
    await t.test(
      'should integrate deduplication into sync process',
      async () => {
        ...
        syncManager.audiobookshelf.getUserLibraryBooks = () =>
          Promise.resolve(duplicateBooks);
        ...
        assert.ok(
          result.books_processed < duplicateBooks.length,
          'Should process fewer than total books',
        );
      },
    );
  });
  ...
  await t.test('Logging and Monitoring', async t => {
    ...
    await t.test('should log duplicate detection', () => {
      ...
      const duplicateMessage = debugMessages.find(
        m => m.message === 'Duplicate book detected',
      );
