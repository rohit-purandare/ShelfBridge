...
test('Double-Processing Fixes Verification', async t => {
  await t.test('Book Deduplication Fix', async () => {
    ...
    assert.strictEqual(
      bookIds.filter(id => id === 'book1').length,
      1,
      'Should only have one book1',
    );
  });
  ...
  await t.test('Processing Tracking Fix', async () => {
    ...
    assert.ok(
      ...
      'Should have processing tracking set',
    );
    assert.strictEqual(
    ...
    assert.ok(
      ...
      'Book should not be tracked initially',
    );
