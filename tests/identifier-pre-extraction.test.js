...
test('Identifier Pre-Extraction Timing Fix', async (t) => {
  await t.test('Pre-extraction prevents race condition timing window', async (t) => {
    ...
    logger.info = (message, data) => {
      if (message.includes('Successfully marked') && message.includes('as completed')) {
    ...
    logger.warn = (message, data) => {
      if (message.includes('No ISBN/ASIN found') && message.includes('using fallback identifier')) {
    ...
    await t.test('should extract identifiers before API call', async () => {
      ...
      assert.ok(fallbackLog.timestamp <= successLog.timestamp, 
  ...
  await t.test('Cache update happens immediately after success', async (t) => {
    ...
    await t.test('should cache immediately after API success', async () => {
      ...
      assert.ok(timeDifference >= 0 && timeDifference < 100, 
  ...
  await t.test('Identifier types are handled correctly', async (t) => {
    ...
    await t.test('should create fallback identifiers consistently', async () => {
      ...
      const originalWarn = logger.warn;
      logger.warn = (message, data) => {
        if (message.includes('using fallback identifier') && data.fallbackIdentifier) {
      ...
      assert.ok(expectedPattern.test(fallbackIdentifier), 
        `Fallback identifier should be clean alphanumeric: ${fallbackIdentifier}`);
      
      assert.ok(fallbackIdentifier.includes(':'), 'Should contain colon separator');
    });
  });
});