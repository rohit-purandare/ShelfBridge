...
describe('EditionSelector', () => {
  ...
  describe('selectBestEdition', () => {
    ...
    it('should prefer audiobook edition for audiobook users', () => {
      ...
      const result = selectBestEdition(mockBookResult, audiobookMetadata, 'audiobook');

      expect(result).not.toBeNull();
      expect(result.edition.id).toBe('edition_audiobook');
      expect(result.edition._editionScore.breakdown.format.reason).toContain('Perfect audiobook format match');
    });
    ...
    it('should prefer ebook edition for ebook users', () => {
      ...
      expect(result.edition._editionScore.breakdown.format.reason).toContain('Perfect ebook format match');
    });

    it('should fall back to ebook when audiobook user has no audiobook edition', () => {
      const bookWithoutAudiobook = {
        ...mockBookResult,
        editions: mockBookResult.editions.filter(e => e.id !== 'edition_audiobook')
      };
      ...
      expect(result).not.toBeNull();
      expect(result.edition.id).toBe('edition_ebook');
      expect(result.edition._editionScore.breakdown.format.reason).toContain('Good fallback');
    });
    ...
    it('should fall back to audiobook when ebook user has no ebook edition', () => {
      ...
      const result = selectBestEdition(bookWithoutEbook, { pages: 350 }, 'ebook');
    ...
    it('should consider popularity in edition selection', () => {
      ...
      const result = selectBestEdition(bookWithVariablePopularity, { duration: 43200 }, 'audiobook');

      expect(result.edition.id).toBe('popular_audiobook');
      expect(result.edition._editionScore.breakdown.popularity.usersCount).toBe(1000);
    });
    ...
    it('should score duration matching for audiobooks', () => {
      ...
      expect(result.edition._editionScore.breakdown.duration.reason).toContain('Excellent duration match');
    });
    ...
    it('should score data completeness correctly', () => {
      ...
      const result = selectBestEdition(bookWithCompleteness, { duration: 43200 }, 'audiobook');

      expect(result.edition.id).toBe('complete_edition');
      expect(result.edition._editionScore.breakdown.completeness.reason).toContain('Very complete');
    });

    it('should apply perfect format match bonus', () => {
      const result = selectBestEdition(mockBookResult, { duration: 43200 }, 'audiobook');

      expect(result.edition._editionScore.breakdown.perfectFormatBonus).toBeDefined();
      expect(result.edition._editionScore.breakdown.perfectFormatBonus.score).toBeGreaterThan(0);
    });
    ...
    it('should apply high popularity bonus for very popular editions', () => {
      ...
      const result = selectBestEdition(veryPopularBook, { duration: 43200 }, 'audiobook');

      expect(result.edition._editionScore.breakdown.popularityBonus).toBeDefined();
      expect(result.edition._editionScore.breakdown.popularityBonus.score).toBeGreaterThan(0);
    });
    ...
    it('should cap edition scores at 100 to prevent overflow', () => {
      ...
      const result = selectBestEdition(extremeBook, { duration: 43200 }, 'audiobook');
    ...
    it('should handle missing format information gracefully', () => {
      ...
      const result = selectBestEdition(bookWithMissingFormat, { duration: 43200 }, 'audiobook');

      expect(result).not.toBeNull();
      expect(result.edition._editionScore.breakdown.format.reason).toContain('No format information');
    });
    ...
    it('should handle physical format as last resort', () => {
      ...
      const result = selectBestEdition(physicalOnlyBook, { duration: 43200 }, 'audiobook');

      expect(result).not.toBeNull();
      expect(result.edition.id).toBe('physical_edition');
      expect(result.edition._editionScore.breakdown.format.reason).toContain('Acceptable fallback');
    });
    ...
    it('should sort alternatives by score', () => {
      ...
      for (let i = 1; i < result.alternativeEditions.length; i++) {
        expect(result.alternativeEditions[i-1]._editionScore.totalScore).toBeGreaterThanOrEqual(
