...
describe('Cross-Cache Reference', () => {
  it('should use complete ASIN cache to avoid title/author search when title/author cache is incomplete', async () => {
    ...
    try {
      ...
      console.log(
        `  Title/author cache: ${titleAuthorId} (incomplete - edition_id: null)`,
      );
      ...
      if (bestCompleteCache) {
        const progressChanged = await bookCache.hasProgressChanged(
          userId,
          bestCompleteCache.key,
          title,
          50.0,
          bestCompleteCache.type,
        );

        console.log(
          `  Progress changed via ${bestCompleteCache.type}: ${progressChanged}`,
        );

        if (!progressChanged) {
          console.log(
            `  🎉 OPTIMIZATION: Would skip entirely via ${bestCompleteCache.type} cache`,
          );
        } else {
          console.log(
            `  💡 ENHANCED IDEA: Could reuse ${bestCompleteCache.type} cache edition info`,
          );
          console.log(
            `    - Edition ID: ${bestCompleteCache.cache.edition_id}`,
          );
          console.log(
            `    - Skip TitleAuthorMatcher since we know the edition`,
          );
      ...
      assert.strictEqual(
        !!bestCompleteCache,
        true,
        'Should find complete cache entry',
      );
      assert.strictEqual(
        bestCompleteCache.type,
        'asin',
        'Should find ASIN cache as complete',
      );
