...
describe('Transaction', () => {
  ...
  describe('rollback()', () => {
    it('executes callbacks in LIFO (reverse) order', async () => {
      ...
      txn.add(async () => { executionOrder.push(1); });
      txn.add(async () => { executionOrder.push(2); });
      txn.add(async () => { executionOrder.push(3); });
    ...
    it('throws the first error encountered during rollback', async () => {
      ...
      txn.add(async () => { throw new Error('Third error'); });
      txn.add(async () => { throw new Error('Second error'); });
      txn.add(async () => { throw new Error('First error'); }); // This executes first
    ...
    it('logs errors using provided logger', async () => {
      ...
      txn.add(async () => { throw new Error('Rollback failed'); });
    ...
    it('handles logger without error method gracefully', async () => {
      ...
      txn.add(async () => { throw new Error('Test error'); });
    ...
    it('uses console as default logger', async () => {
      ...
      try {
        txn.add(async () => { throw new Error('Default logger test'); });
    ...
    it('handles mixed sync and async callbacks', async () => {
      ...
      txn.add(() => { results.push('sync-1'); }); // Sync callback
      txn.add(async () => { 
        await new Promise(resolve => setTimeout(resolve, 5));
        results.push('async-2'); 
      });
      txn.add(() => { results.push('sync-3'); }); // Sync callback
  ...
  describe('integration scenarios', () => {
    it('supports typical usage pattern with success path', async () => {
      ...
      txn.add(async () => { cleanupActions.push('revert-step-3'); });
      txn.add(async () => { cleanupActions.push('revert-step-2'); });
      txn.add(async () => { cleanupActions.push('revert-step-1'); });
    ...
    it('supports typical usage pattern with failure path', async () => {
      ...
      assert.deepEqual(cleanupActions, ['revert-step-3', 'revert-step-2', 'revert-step-1']);
      assert.equal(txn._committed, true);
      assert.equal(txn._rollbackCallbacks.length, 0);
    });
    ...
    it('handles complex scenario with partial rollback failures', async () => {
      ...
      txn.add(async () => { successfulCleanups.push('cleanup-1'); });
      txn.add(async () => { 
        successfulCleanups.push('cleanup-2-before-error');
        throw new Error('Cleanup 2 failed'); 
      });
      txn.add(async () => { successfulCleanups.push('cleanup-3'); });
