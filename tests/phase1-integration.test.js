...
describe('Phase 1 Integration Tests', () => {
  ...
  describe('TimestampFormatter + CacheKeyGenerator Integration', () => {
    it('should work together for book processing workflow', () => {
      ...
      const displayStarted = timestampFormatter.formatForDisplay(
        mockBookData.started_at,
      );
      const displayFinished = timestampFormatter.formatForDisplay(
        mockBookData.finished_at,
      );
      const displayLastListened = timestampFormatter.formatForDisplay(
        mockBookData.last_listened_at,
      );

      const apiStarted = timestampFormatter.formatForHardcover(
        mockBookData.started_at,
      );
      const apiFinished = timestampFormatter.formatForHardcover(
        mockBookData.finished_at,
      );
      ...
      const cacheKeys = CacheKeyGenerator.generatePossibleKeys(
        identifiers,
        mockHardcoverMatch,
      );
      ...
      assert(
        displayStarted.includes('2024'),
        'Display timestamp should include year',
      );
      assert(
        displayStarted.includes('EST') ||
          displayStarted.includes('-05') ||
          displayStarted.includes('-04'),
        'Should show timezone',
      );
      assert(
        displayFinished.includes('2024'),
        'Finished timestamp should be formatted',
      );
      assert(
        displayLastListened.includes('2024'),
        'Last listened should be formatted',
      );

      assert(
        apiStarted.match(/^\d{4}-\d{2}-\d{2}$/),
        'API started should be YYYY-MM-DD',
      );
      assert(
        apiFinished.match(/^\d{4}-\d{2}-\d{2}$/),
        'API finished should be YYYY-MM-DD',
      );
      ...
      assert.strictEqual(
        storageKey.identifierType,
        'asin',
        'Should prefer ASIN for storage',
      );
      assert.strictEqual(
        storageKey.identifier,
        'B001234567',
        'Should use correct ASIN',
      );
      ...
      assert(
        syntheticKey.identifierValue.includes('user-book-456'),
        'Synthetic key should include user book ID',
      );
      assert(
        syntheticKey.identifierValue.includes('edition-789'),
        'Synthetic key should include edition ID',
      );
    });
    ...
    it('should handle edge cases consistently across utilities', () => {
      ...
      edgeCases.forEach((testCase, index) => {
        ...
        const displayResult = timestampFormatter.formatForDisplay(
          testCase.timestamp,
        );
        const apiResult = timestampFormatter.formatForHardcover(
          testCase.timestamp,
        );
        ...
        if (testCase.timestamp === null || testCase.timestamp === undefined) {
          assert.strictEqual(
            displayResult,
            'N/A',
            `Test case ${index}: Display should return N/A for null/undefined`,
          );
          assert.strictEqual(
            apiResult,
            null,
            `Test case ${index}: API should return null for null/undefined`,
          );
        } else if (
          testCase.timestamp === 'invalid' ||
          testCase.timestamp === 'malformed-date'
        ) {
          assert(
            displayResult.includes('Invalid') ||
              displayResult.includes('Error'),
            `Test case ${index}: Display should indicate error`,
          );
          assert.strictEqual(
            apiResult,
            null,
            `Test case ${index}: API should return null for invalid dates`,
          );
        }
        ...
        assert(
          ...
          `Test case ${index}: Fallback key should be string`,
        );
        assert(
          fallbackKey.length > 0,
          `Test case ${index}: Fallback key should not be empty`,
        );
        assert.strictEqual(
          syntheticKey.identifierValue,
  ...
  describe('Performance and Memory Integration', () => {
    it('should handle large batches efficiently', () => {
      ...
      assert(
        processingTime < 1000,
        `Processing ${batchSize} items should take less than 1 second, took ${processingTime}ms`,
      );
      ...
      results.forEach((result, index) => {
        assert(
          result.displayTime.length > 10,
          `Item ${index}: Display time should be formatted`,
        );
        assert(
          result.apiTime.match(/^\d{4}-\d{2}-\d{2}$/),
          `Item ${index}: API time should be YYYY-MM-DD`,
        );
        assert(
          result.fallbackKey.length > 0,
          `Item ${index}: Fallback key should exist`,
        );
        assert.strictEqual(
          result.storageKey.identifierType,
          'asin',
          `Item ${index}: Should prefer ASIN`,
        );
      });

      console.log(
        `✅ Performance test: Processed ${batchSize} items in ${processingTime.toFixed(2)}ms (${(processingTime / batchSize).toFixed(2)}ms per item)`,
      );
    });
    ...
    it('should handle repeated operations without crashing', () => {
      ...
      console.log(
        `✅ Stability test: ${iterations} operations completed without errors`,
      );
    });
  });
    ...
    it('should gracefully handle errors across utility boundaries', () => {
      ...
      errorScenarios.forEach((scenario, index) => {
        ...
        assert.doesNotThrow(() => {
          ...
          assert(
            typeof displayResult === 'string',
            `Scenario ${index}: Display result should be string`,
          );
          assert(
            apiResult === null || typeof apiResult === 'string',
            `Scenario ${index}: API result should be string or null`,
          );
        ...
        assert.doesNotThrow(() => {
          ...
          assert(
            Array.isArray(keys),
            `Scenario ${index}: Keys should be array`,
          );
  ...
  describe('Cross-Utility Data Flow', () => {
    it('should maintain data consistency when utilities are chained', () => {
      ...
      const normalizedIdentifiers = {
        isbn: CacheKeyGenerator.normalizeIdentifier(
          rawIdentifiers.isbn,
          'isbn',
        ),
        asin: CacheKeyGenerator.normalizeIdentifier(
          rawIdentifiers.asin,
          'asin',
        ),
      };
      ...
      const formattedTimestamps = {
        started_display: timestampFormatter.formatForDisplay(
          mockBookSync.absBook.started_at,
        ),
        started_api: timestampFormatter.formatForHardcover(
          mockBookSync.absBook.started_at,
        ),
        last_listened_display: timestampFormatter.formatForDisplay(
          mockBookSync.absBook.last_listened_at,
        ),
        last_listened_local: timestampFormatter.convertUTCMillisToLocal(
          mockBookSync.absBook.last_listened_at,
        ),
      };
      ...
      assert.strictEqual(
        normalizedIdentifiers.isbn,
        '9780123456789',
        'ISBN should be normalized (hyphens removed)',
      );
      assert.strictEqual(
        normalizedIdentifiers.asin,
        'B001234567',
        'ASIN should be normalized (uppercase)',
      );

      assert.strictEqual(
        storageKey.identifier,
        'B001234567',
        'Storage should use normalized ASIN',
      );
      assert.strictEqual(
        storageKey.identifierType,
        'asin',
        'Storage should prefer ASIN',
      );

      assert(
        formattedTimestamps.started_display.includes('2024-01-15'),
        'Display timestamp should include date',
      );
      assert.strictEqual(
        formattedTimestamps.started_api,
        '2024-01-15',
        'API timestamp should be YYYY-MM-DD',
      );

      assert(
        fallbackKey.includes('testbook'),
        'Fallback key should include normalized title',
      );
      assert(
        fallbackKey.includes('unknownauthor'),
        'Fallback key should include normalized author',
      );
      ...
      assert(
        asinKey && asinKey.priority < isbnKey.priority,
        'ASIN should have higher priority than ISBN',
      );
      assert.strictEqual(
        asinKey.key,
        'B001234567',
        'ASIN key should use normalized value',
      );

      console.log(
        '✅ Data flow validation: All utilities maintain data consistency',
      );
    });
  });
